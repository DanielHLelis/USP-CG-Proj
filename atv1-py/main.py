...
class Ship(Entity):
    ...
    def update(self, dt):
        ...
        if self.accelerating != 0:
            self.acceleration += self.ACCELERATING_RATE * dt * np.sign(self.accelerating)
            self.acceleration = np.clip(self.acceleration, self.MIN_SPEED, self.MAX_SPEED)
...
class Asteroid(Entity):
    ...
    @staticmethod
    def make_random():
        color = colorsys.hls_to_rgb(np.random.uniform(0, 1), np.random.uniform(0.7, 0.8), np.random.uniform(0.8, 0.9))
    ...
    def __init__(self, vertices:np.array = None, acc_x: float = 0.0, acc_y: float = 0.0, **kwargs):
        if vertices is None:
            vertices = np.array([
                    0.0, 0.5,
                    -0.5, -0.5,
